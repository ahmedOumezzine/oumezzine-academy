...
public sealed class EfAdminCourseQueries(ApplicationDbContext db) : IAdminCourseQueries
{
    ...
    public async Task<IReadOnlyList<AdminCourseCategoryOptionDto>> GetCategoryOptionsAsync(CancellationToken token=default){var rows=await db.StudyCourseCategories.AsNoTracking().Select(c=>new{c.Id,DisplayName=c.Translations.Where(t=>t.LanguageCode=="fr").Select(t=>t.Title).FirstOrDefault()??c.Translations.Where(t=>t.LanguageCode=="en").Select(t=>t.Title).FirstOrDefault()??"Catégorie sans traduction"}).ToListAsync(token);return rows.OrderBy(x=>x.DisplayName).ThenBy(x=>x.Id).Select(x=>new AdminCourseCategoryOptionDto(x.Id,x.DisplayName)).ToList();}
