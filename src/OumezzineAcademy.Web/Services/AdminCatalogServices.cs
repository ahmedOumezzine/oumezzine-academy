...
public sealed class AdminCategoryService(IAdminCategoryQueries queries, IAdminCategoryPersistence persistence, IStudyLmsCacheInvalidator cache)
{
    ...
    public async Task SaveAsync(CategoryEditViewModel model, CancellationToken token)
    { static AdminCategoryTranslationDto Map(CategoryTranslationInput x) => new(x.Title, x.Slug, x.Summary, x.MetaTitle, x.MetaDescription, x.PublicationStatus); await persistence.SaveAsync(new AdminCategorySaveCommand(model.Id == Guid.Empty ? null : model.Id, Map(model.French), Map(model.English)), token); await cache.InvalidatePublicAsync(token); }

    public async Task<AdminCategoryDeleteStatus> DeleteAsync(Guid id, CancellationToken token)
    { var result = await persistence.DeleteAsync(id, token); if (result == AdminCategoryDeleteStatus.Deleted) await cache.InvalidatePublicAsync(token); return result; }
}
    ...
    IStudyLmsCacheInvalidator cache)
{
    ...
    public async Task<string?> UploadThumbnailAsync(Guid id, IFormFile file, CancellationToken token)
    { await using var stream = file.OpenReadStream(); var result = await mediaCommands.UploadAsync(id, new MediaUpload(file.FileName, file.ContentType, file.Length, stream), token); if (result.Found) await cache.InvalidatePublicAsync(token); return result.StoredPath; }

    public async Task<bool> DeleteThumbnailAsync(Guid id, CancellationToken token)
    { var result = await mediaCommands.RemoveAsync(id, token); if (result) await cache.InvalidatePublicAsync(token); return result; }
    ...
    public async Task<DeleteCourseResult> DeleteAsync(Guid id, CancellationToken token)
    { var result = await deleteCommands.DeleteAsync(id, token); if (result.Success) await cache.InvalidatePublicAsync(token); return result; }
