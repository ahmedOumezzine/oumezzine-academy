...
public sealed class AdminQuestionService(IAdminQuestionQueries queries, IAdminQuestionCommands commands, IStudyLmsCacheInvalidator? cache = null)
{
    ...
    public async Task MoveAsync(Guid courseId,Guid chapterId,Guid quizId,Guid id,int direction,CancellationToken token){await commands.MoveAsync(new AdminQuestionMoveCommand(courseId,chapterId,quizId,id,direction),token);await _cache.InvalidatePublicAsync(token);}
    public async Task DeleteAsync(Guid courseId,Guid chapterId,Guid quizId,Guid id,CancellationToken token){if(await commands.DeleteAsync(courseId,chapterId,quizId,id,token)==AdminQuestionDeleteStatus.NotFound)throw new KeyNotFoundException();await _cache.InvalidatePublicAsync(token);}
}
