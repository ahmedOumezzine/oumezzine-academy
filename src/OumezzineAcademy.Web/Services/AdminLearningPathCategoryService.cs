...
public sealed class AdminLearningPathCategoryService(IAdminLearningPathCategoryQueries queries,IAdminLearningPathCategoryCommands commands,CacheInvalidator? cache=null)
{
 ...
 public async Task SaveAsync(LearningPathCategoryEditViewModel m,CancellationToken t){var r=await commands.SaveAsync(new AdminLearningPathCategorySaveCommand(m.Id==Guid.Empty?null:m.Id,m.French.Title,m.French.Slug,m.French.Summary,m.French.MetaTitle,m.French.MetaDescription,m.French.PublicationStatus,m.English.Title,m.English.Slug,m.English.Summary,m.English.MetaTitle,m.English.MetaDescription,m.English.PublicationStatus),t);await _cache.InvalidatePublicAsync(t);}
 public async Task DeleteAsync(Guid id,CancellationToken t){var r=await commands.DeleteAsync(id,t);if(r==AdminLearningPathCategoryDeleteStatus.NotFound)throw new KeyNotFoundException();if(r==AdminLearningPathCategoryDeleteStatus.InUse)throw new InvalidOperationException("Cette catégorie contient des parcours et ne peut pas être supprimée.");await _cache.InvalidatePublicAsync(t);}
}
