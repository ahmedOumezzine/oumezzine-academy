...
public sealed class CourseCatalogService(ICourseCatalogQueries queries, ICurrentLanguageService language) : ICourseCatalogService
{
    ...
    { var x = await queries.GetHomeAsync(Lang); return new() { CourseCount = x.CourseCount, CategoryCount = x.CategoryCount, LearningPathCount = x.LearningPathCount, LessonCount = x.LessonCount, LatestCourses = x.LatestCourses.Select(Card).ToList(), TopCategories = x.TopCategories.Select(Category).ToList(), LatestLearningPaths = x.LatestLearningPaths.Select(Path).ToList() }; }

    public async Task<CourseSearchViewModel> SearchCoursesAsync(string? q, string? category, StudyLevel? level, string? sort, string? view, int page, int pageSize)
    { var x = await queries.SearchCoursesAsync(new(q, category, level, sort, page, pageSize, Lang)); return new() { Q = q, Category = category, Level = level, Sort = sort ?? "recent", View = view == "list" ? "list" : "grid", Page = x.Page, PageSize = x.PageSize, TotalItems = x.TotalItems, Courses = x.Items.Select(Card).ToList() }; }
