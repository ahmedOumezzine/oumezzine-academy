...
public sealed class AdminLearningPathService
{
    ...
    public AdminLearningPathService(IAdminLearningPathMediaCommands media, IAdminLearningPathQueries queries, IAdminLearningPathCoreCommands core, IAdminLearningPathCommands composition, IStudyLmsCacheInvalidator? cache = null) { _media = media; _queries=queries; _core=core; _composition=composition; _cache = cache ?? new NullStudyLmsCacheInvalidator(); }
    ...
    public async Task AddCourseAsync(Guid pathId,Guid courseId,CancellationToken token){await _composition.AddCourseAsync(pathId,courseId,token);await _cache.InvalidatePublicAsync(token);}
    public async Task SynchronizeCoursesAsync(Guid pathId,IReadOnlyCollection<Guid> courseIds,CancellationToken token){await _composition.SynchronizeCoursesAsync(pathId,courseIds,token);await _cache.InvalidatePublicAsync(token);}
    public async Task MoveCourseAsync(Guid pathId,Guid id,int direction,CancellationToken token){await _composition.MoveCourseAsync(pathId,id,direction,token);await _cache.InvalidatePublicAsync(token);}
    public async Task RemoveCourseAsync(Guid pathId,Guid id,CancellationToken token){await _composition.RemoveCourseAsync(pathId,id,token);await _cache.InvalidatePublicAsync(token);}
}
