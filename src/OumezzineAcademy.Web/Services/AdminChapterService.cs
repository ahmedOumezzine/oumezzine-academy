...
public sealed class AdminChapterService(IAdminChapterQueries queries, IAdminChapterPersistence persistence, IStudyLmsCacheInvalidator cache, IAdminChapterMediaCommands media)
{
    ...
    public async Task<ChapterEditViewModel?> GetFormAsync(Guid courseId, Guid? id, CancellationToken token)
    ...
    public async Task SaveAsync(ChapterEditViewModel m, CancellationToken token)
    { static AdminChapterTranslationDto M(ChapterTranslationInput x) => new(x.Title, x.Summary, x.PublicationStatus); var r = await persistence.SaveAsync(new AdminChapterSaveCommand(m.Id == Guid.Empty ? null : m.Id, m.CourseId, m.Order, M(m.French), M(m.English)), token); if (!r.Success) throw new InvalidOperationException(r.Error); m.Id = r.ChapterId; await cache.InvalidatePublicAsync(token); }

    public async Task MoveAsync(Guid courseId, Guid id, int direction, CancellationToken token)
    { await persistence.MoveAsync(courseId, id, direction, token); await cache.InvalidatePublicAsync(token); }

    public async Task DeleteAsync(Guid courseId, Guid id, CancellationToken token)
    { var r = await persistence.DeleteAsync(courseId, id, token); if (!r.Success) throw new InvalidOperationException(r.Message); await cache.InvalidatePublicAsync(token); }

    public async Task<string> UploadImageAsync(Guid courseId, Guid id, IFormFile file, CancellationToken token)
    { await using var stream = file.OpenReadStream(); var path = await media.UploadAsync(courseId, id, new MediaUpload(file.FileName, file.ContentType, file.Length, stream), token); if (path is null) throw new KeyNotFoundException(); return path; }
}